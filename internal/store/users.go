...
func (s *UserStore) Create(ctx context.Context, tx *sql.Tx, user *User) error {
	query := `
		INSERT INTO users (username, password, email, role_id) VALUES($1, $2, $3, (SELECT id FROM roles WHERE name = $4)) RETURNING id, created_at, updated_at
	`
...
func (s *UserStore) Activate(ctx context.Context, token string) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		...
		if err := s.deleteUserFromInvitations(ctx, tx, user.ID); err != nil {
			return err
		}
...
func (s *UserStore) deleteUserFromInvitations(ctx context.Context, tx *sql.Tx, userID int64) error {
